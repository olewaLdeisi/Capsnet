...
class Dataset_cg(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            ...
            # img = transforms.Normalize((0.7709, 0.5011, 0.2981),(0.1569, 0.1470, 0.1451))(img)
            # mark1 = transforms.Normalize((0.2407,), (0.2723,))(mark1)
        ...
        img_5c = torch.cat((img,*mark),0)
        img_5c = transforms.ToPILImage()(img_5c)
        img_5c.save(os.path.join('./data_5c/img/' , self.img[index]),"PNG")
...
class OldDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mark1 = transforms.ToTensor()(mark1)
        mark2 = transforms.ToTensor()(mark2)
        ...
        # img_5c = transforms.ToPILImage()(img)
        # img_5c.save(os.path.join('./data_5c/img/' , self.img[index]),"PNG")
        label = eval(self.label[index])
        # 返回图片，对应标签
        # print(self.img[index],self.labellist.index(label))
