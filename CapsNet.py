"""
CapsNet Model
from: https://github.com/adambielski/CapsNet-pytorch/blob/master/net.py
"""
...
class CapsLayer(nn.Module):
    def __init__(self, input_caps, input_dim, output_caps, output_dim, routing_module,block_size=3,drop_prob=0.2,block=True):
        ...
        if block:
            ...
            self.dropout = nn.Dropout2d(p=self.drop_prob)
    ...
    def forward(self, caps_output):
        ...
        u_predict = self.dropout(u_predict) # 与源码不同的地方
        # 在动态路由之前加入dropout层
...
class CapsNet(nn.Module):
    def __init__(self, routing_iterations, n_classes=10,block_size=3,drop_prob=2,img_size=224,block=True):
        ...
        self.num_primaryCaps = 32 * npc * npc
        routing_module = AgreementRouting(self.num_primaryCaps, n_classes, routing_iterations)
        self.digitCaps = CapsLayer(self.num_primaryCaps, 8, n_classes, 16, routing_module,block_size=block_size,drop_prob=drop_prob,block=block)

    def forward(self, input):
        x = self.conv1(input) # 28*28 -> 20*20
        x = self.bn(x)  # 与源码不同
        x = F.relu(x)
        if self.block:
            x = self.dropout(x)
