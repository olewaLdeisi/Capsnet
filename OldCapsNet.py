...
class CapsLayer(nn.Module):
    ...
    def forward(self, caps_output):
        ...
        u_predict = self.dropout(u_predict)
...
class PrimaryCapsLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        a = out.detach().view(-1,8).cpu().numpy()
        out = squash(out)
        b = out.detach().view(-1,8).cpu().numpy()
        a = 1
        return out


class CapsNet(nn.Module):
    def __init__(self, routing_iterations, n_classes=2,block_size=3,drop_prob=2,img_size=224,block=True):
...
class ReconstructionNet(nn.Module):
    def __init__(self, n_dim=16, n_classes=2, img_size=224):
        ...
        self.fc3 = nn.Linear(2048, 28 * 28 * 3)
        self.n_dim = n_dim
